...
func ParseWorkspaceMode(raw string) (WorkspaceMode, error) {
	...
	default:
		return "", fmt.Errorf("invalid balda.workspace.mode %q: expected one of on, off, auto", raw)
	}
}
...
func ResolveWorkspaceEnabled(
	...
	case WorkspaceModeOn:
		if isGitRepo != nil && !isGitRepo(ctx, workingDir) {
			return "", false, fmt.Errorf("balda.workspace.mode %q requires a git repository at balda.working_dir %q", mode, workingDir)
		}
